    ...
    element.addEventListener("change",(e)=>{
        ...
        box.addEventListener("click",(e)=>{
            ...
            showValus= showValus.filter(el=>{
                return +el.attributes["data-number"].value !== +box.attributes["data-number"].value
            })
            if(!showValus.length){
        ...
        box.append(span)
        box.append(icon)
        if(!showValus.some(el=>Number(el.attributes["data-number"].value)===i)){
            showValus.push(box)
        }
        }else {
            ...
            showValus= showValus.filter((el)=>{
                ...
                for (let i = 0; i < box.length; i++) {
                    const element = box[i];
                    if(+el.attributes["data-number"].value === +element.attributes["data-number"].value){
                        element.remove()
                    }
                }
            }
